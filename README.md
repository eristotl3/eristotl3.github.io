...
日本語が正本で、英語は各要素の `data-en` 属性に HTML で入っています。**ファイルは分けていません。**
分けると構造を直したときに片方だけ古くなり、どちらが正しいのか分からなくなるためです。

文章を直すときは、要素の中身と `data-en` の両方を直してください。隣り合っているので忘れにくいはずです。

```html
<h2 class="patch__title" data-en="About">私について</h2>
```

- `data-en` には HTML を書けます（`<br>` や `<b>` が使えます）
- 英語に訳す必要がないもの（GitHub、TOEFL、Rust など）には `data-en` を付けていません
- 切り替えた言語はブラウザに記憶されます
- **`?lang=en` を付けた URL は、開いた瞬間から英語です。** 英語話者にリンクを渡すときはこちらを使ってください

```
https://eristotl3.github.io/?lang=en
```

狭い画面（600px 未満）ではページ内リンクを隠しています。4項目が収まらず、横スクロールしないと
半分見えない状態になるためです。言語トグルと名前は常に出ます。
